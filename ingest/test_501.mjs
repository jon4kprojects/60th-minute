...
const g = F.createGame({ club:'Manchester United F.C.', metric:'apps', names:['Jon','James'] });
...
const r1 = say('Giggs');                       // 672 -> over 180 -> 0
ok(r1.status === 'over-max' && r1.score === 0, 'Giggs (672) scores 0, over the 180 max');
const r2 = say('Beckham');
ok(r2.raw > 0, 'Beckham returns a real number');
const r3 = say('Thierry Henry');               // never played for Man Utd
ok(r3.status === 'ineligible', 'Henry rejected as ineligible for Man Utd');
const r4 = say('Beckham');
ok(r4.status === 'duplicate', 'Beckham cannot be named twice');
...
console.log('\n=== playable depth ===');
for (const c of ['Manchester United F.C.','Arsenal F.C.','Celtic F.C.','West Ham United F.C.']) {
